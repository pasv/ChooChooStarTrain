...
# TODO: make this into part of the ini config
startupapps=["C:\Program Files (x86)\StarC" + "raft II\StarC" + "raft II.exe", \
...
# TODO: add this as well to ini.
# change as needed
larvasound="C:\Program Files (x86)\OpenOffice.org 3\Basis\share\gallery\sounds\strom.wav"
key=_winreg.OpenKey(_winreg.HKEY_CURRENT_USER, "SOFTWARE\Razer\Sta" + "rcr" + "aft2") # no thanks google
injects = 0
auto_inject = False
 ...
 if(_winreg.QueryValueEx(key, monitored_value)[0]==1):
  ...
  if(auto_inject == True):
   for n in range(num_of_base):
    ...
    click(960, 540)
    time.sleep(0.1)
    # TODO: perhaps it would be better here to set a temporary camera location before going into the loop then return to it and the selection
